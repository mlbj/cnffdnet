'''
"Implementation of Spectral Normalization as proposed by Miyato et al. (2018).

https://arxiv.org/abs/1802.05957

Applies SN to convolutional layers to constrain the Lipschitz constant to 
$\sigma$ (default = 1.0)."
'''
...
class ConvSpectralNorm(object):
    def __init__(self, name='weight', sigma=1.0, n_power_iterations=1, dim=0, eps=1e-12):
        self.name = name
        self.sigma = sigma
        self.dim = dim
    ...
    def compute_weight(self, module):
        ...
        with torch.no_grad():
            for _ in range(self.n_power_iterations):
                ...
                v = normalize(conv_transpose2d(u,# .flip(2,3),
                                     weight_mat,
                                     padding=1),
        ...
        weight=weight*torch.min(torch.tensor(1.0), (self.sigma/cur_sigma))
    ...
    @staticmethod
    def apply(module, name, sigma, n_power_iterations, dim, eps, patch_shape=(64,64)):
        ...
        u = normalize(weight.new_empty(1, C_out, *patch_shape).normal_(0, 1), eps=fn.eps)
...
def conv_spectral_norm(module, name='weight', sigma=1.0, n_power_iterations=1,
                       eps=1e-12, dim=None,patch_shape=(66,66)):
